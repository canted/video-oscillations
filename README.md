...
- Tap the spectrum to add a color stop, or choose **add stop** to split the largest gap (the third stop starts at 50%). Up to 16 stops are supported.
- Drag a stop to position it, or use its **position %** field. Stops stay ordered and cannot overlap. Arrow keys move the focused stop by 1%, Shift+arrow by 10%; Home/End move it as far as its neighbors allow.
- Tap a stop or choose it in the **edit** menu to change its rainbow swatch, custom color, or shade. The shade slider runs from black (0%), through the exact color (50%), to white (100%).
- **Delete stop** removes the selected stop (at least two remain). Delete/Backspace also work on focused stops.
